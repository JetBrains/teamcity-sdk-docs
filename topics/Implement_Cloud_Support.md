...
This page explains how to implement cloud support in TeamCity. You can use the open-source [Google Cloud Agents](https://github.com/JetBrains/teamcity-google-agent) plugin as a reference.
...
2. Register the implemented `CloudClientFactory` class in `jetbrains.buildServer.clouds.CloudRegistrar`. You can get an instance of the CloudRegistrar interface in the constructor of your plugin's [Spring bean](getting-started-with-plugin-development.md#view-the-project-structure).
...
* `canStartNewInstance(CloudImage image)` &mdash; This method is called to check whether it is possible to start one mode instance of the provided image. If this method returns `false`, TeamCity schedules the `startNewInstance` method.
